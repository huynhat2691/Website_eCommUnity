...
const AdminAddEvents = () => {
  ...
  useEffect(() => {
    if (adminEvents && adminEvents.length >= 10) {
      toast.warning("Số lượng sản phẩm tham gia flash sale chỉ được <= 10");
      navigate("/admin/admin-events");
    }
  }, [adminEvents, navigate]);
  ...
  useEffect(() => {
    ...
    if (success) {
      ...
      navigate("/admin/admin-events");
      dispatch(resetAdminAddEventSuccess());
    }
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(adminAddEvent(newForm));
  };
